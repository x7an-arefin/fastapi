...
export class HealthAndMetricsPlugin implements IPlugin {
  ...
  async afterModulesRegistered(_app: Application, hono: Hono): Promise<void> {
    hono.get('/health', async (c) => {
      ...
      // KV check
      try {
        const kv = env['AUTH_SESSION_KV'] as KVNamespace | undefined;
        if (kv) {
